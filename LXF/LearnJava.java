...
public class LearnJava {
    ...
    static ProcessResult runJavaProgram(String code) throws IOException, InterruptedException {
        ...
        String[] commandJavac = new String[] { "javac", "Main.java"};
        String[] commandJava = new String[] { "java", "Main"};
        System.out.println(String.format("cd %s\n%s\n%s", pwd.toString()
                , String.join(" ", commandJavac)
                , String.join(" ", commandJava)));

        ProcessBuilder pb = new ProcessBuilder().command(commandJavac).directory(pwd);
        pb.redirectErrorStream(true);
        Process p = pb.start();
        if (p.waitFor() == 0) {
            p = pb.command(commandJava).start();
        }
        return getProcessResult(p);
    }

    private static ProcessResult getProcessResult(Process p) throws InterruptedException, IOException {
        if (p.waitFor(3, TimeUnit.SECONDS)) {
            ...
            System.err.println(String.format("Error: process %s timeout. destroy forcibly.", p));
            p.destroyForcibly();
    ...
    static class CodeHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if ("GET".equals(method)) {
                ...
                if (!body.startsWith("code=")) {
                    ...
                    String code = URLDecoder.decode(body.substring(5), "UTF-8");
        ...
        void sendData(HttpExchange exchange, String s) throws IOException {
            String origin = exchange.getRequestHeaders().getOrDefault("Origin", Collections.singletonList("https://www.liaoxuefeng.com"))
                    .get(0);
